    ...
    <a-tabs v-model:activeKey="scanInfo.scan.tabKey" animated size="large">
      ...
      <a-tab-pane :key="1">
        ...
        <a-flex gap="large" vertical>
          ...
          <a-card hoverable>
            <h2>
              <span class="bi bi-folder2-open"> </span> Select Input Folder:
            </h2>
            <br />
            ...
            <a-flex horizontal justify="start" align="center">
              ...
              <a-button
                size="large"
                @click="handleGoToFolder(scanInfo.scan.folderPath)"
                ><i class="bi bi-aspect-ratio" style="margin-right: 5px"></i>
                View</a-button
              >
              ...
              <a-collapse-panel header="Preprocess" class="customStyle1">
                    ...
                    <a-flex align="left" gap="large" class="resizeStyle">
                      <a-row>
                        <a-radio
                          ...
                          :checked="
                            preprocessInfo.preprocess.selectedRadio == 'ratio'
                          "
                      ...
                      <a-row>
                        <a-radio
                          ...
                          :checked="
                            preprocessInfo.preprocess.selectedRadio == 'size'
                          "
                          >Resize:
                        </a-radio>
                        <a-input-number
                    ...
                    <a-flex align="center" gap="large">
                      ...
                      >
                        ...
                        <a-radio
                          ...
                          >RGB</a-radio
                        >
                        <a-radio value="BW" size="large" style="font-size: 18px"
                          >BW</a-radio
                        >
                        <a-radio
                          value="HSV"
                ...
                >
                  <template #description>
                    <p>
                      Output:<a @click="handleGoToFolder(outputPath)">{{
                        outputPath
                      }}</a>
                    </p>
                  </template>
          ...
          <a-card hoverable>
            <h3>
              Input file count:
              <span style="font-weight: bold">{{ scanInfo.scan.length }}</span>
            </h3>
          </a-card>
          ...
          <a-card hoverable>
            <a-flex gap="middle" vertical>
              <h2 style="margin-block: 10px">
                <span class="bi bi-check2-square"></span> Select Biometircs
                Modality / Engine:
              </h2>
          ...
          <a-row justify="center" style="margin-block: 2rem">
            <a-button
              ...
              @click="submitScan2"
              :loading="
                scanInfo.process.taskStatus.filter((item) => item.status != 2)
                  .length > 0
              "
        ...
        <a-flex gap="large" vertical>
          ...
          <a-card hoverable>
            <a-flex vertical gap="middle">
              ...
              <a-collapse
                v-model:activeKey="scanInfo.scan.activeKeys"
                :bordered="false"
              >
                <!-- upload section -->
                <a-collapse-panel
                ...
                >
                  ...
                  <a-upload-dragger
                    ...
                    :disabled="
                      scanInfo.scan.disableUploader ||
                      scanInfo.scan.inputType.length == 0
                    "
                    :showUploadList="false"
                  >
                    ...
                    <p class="ant-upload-text">
                      Click here, or drag a folder here to upload the samples/
                      Maximum of a 1000 files.
                    </p>
                    <p class="ant-upload-hint">
                      Please do not leave the current page when uploading the
                      directory.
                    </p>
                  </a-upload-dragger>
              ...
              <a-flex gap="middle" vertical>
                ...
                <a-select
                  ...
                  ><template #suffixIcon
                    ><i class="bi bi-file-earmark-break"></i></template
                ></a-select>
            ...
            <a-flex vertical gap="middle">
              ...
              >
                ...
                >
                  <div
                    class="emptyPreview"
                    v-if="scanInfo.scan.validFileList.length == 0"
                  >
                    <a-empty />
                  </div>
                    ...
                    >
                      <template #cover>
                        <a-tooltip :title="item.name">
                          <div
                            v-if="
                              item.name.includes('.wsq') ||
                              item.name.includes('.jp2')
                            "
                          >
                            <img
                              v-if="
                    ...
                    >
                      <template #cover>
                        <a-tooltip :title="item1.name">
                          <div
                            v-if="
                              item1.name.includes('.wsq') ||
                              item1.name.includes('.jp2')
                            "
                          >
                            <img
                              v-if="
          ...
          <a-card hoverable>
            <h3>
              ...
              <span style="font-weight: bold">{{
                scanInfo.scan.validFileList.length
              }}</span>
            </h3>
          </a-card>
          ...
          <a-card hoverable>
            <a-flex vertical gap="middle">
              <h2 style="margin-bottom: 10px">
                <span class="bi bi-check2-square"></span> Select Biometrics
                Modality / Engine:
              </h2>
              <a-cascader
                popupClassName="popupClass"
...
const openNotificationWithIcon = (type: string) => {
  if (type === "stop") {
    ...
    notification["error"]({
      ...
      description:
        "Current operation failed. Please check your input and try again.",
      placement: "top",
    });
  } else if (type === "upload") {
    notification["success"]({
...
const checkInputFolder = async () => {
  ...
  const exts = scanInfo.value.scan.inputType
    .map((ext) => `exts=${ext.toLowerCase()}`)
    .join("&");
  const pattern = scanInfo.value.scan.name
    ? `&pattern=${scanInfo.value.scan.name}`
    : "";
...
const debouncedSearch = (event) => {
  ...
  debounceTimer = setTimeout(() => {
    ...
    if (scanInfo.value.scan.tabKey == 1) {
      ...
      scanInfo.value.scan.validFileList = scanInfo.value.scan.fileList.filter(
        (item) => validateFile(item)
      );
    }
  }, debounceDelay);
};
...
const beforeUpload: UploadProps["beforeUpload"] = async (file) => {
  ...
  if (format) {
    const index = scanInfo.value.scan.fileList.findIndex(
...
const convertToBase64 = async (file) => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      let res = base64WithoutMetadata.padEnd(
        base64WithoutMetadata.length + requiredPadding,
        "="
      );
      resolve(res);
    };
  ...
  if (!scanInfo.value.scan.modality[1]) {
    requestBody = {
      options: {
        ...
        pattern: scanInfo.value.scan.name
          ? transferPatterns(scanInfo.value.scan.name)
          : null,
      },
      input: input,
    };
  ...
  if (e.file.status !== "uploading") {
    ...
    if (res.message) {
      ...
      preprocessInfo.value.preprocess.folderPath =
        res.message.substring(startIndex);
...
const submitPreprocess = async () => {
  const requestBody = {
    ...
    input_format: scanInfo.value.scan.inputType.map((type) =>
      type.toLowerCase()
    ),
    status: 0,
  };
    ...
    await API.authFetch(url2, {
      ...
      .then((data) => {
        ...
        if (data.status == 2 || data.status == -1) {
          ...
          preprocessInfo.value.preprocess.id = "";
          preprocessInfo.value.preprocess.loading = false;
          const index = scanInfo.value.scan.inputType.indexOf(
            preprocessInfo.value.preprocess.type
          );
          if (index === -1) {
            scanInfo.value.scan.inputType.push(
              preprocessInfo.value.preprocess.type
            );
          }
          checkInputFolder();
